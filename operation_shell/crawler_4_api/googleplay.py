...
class GooglePlayAPI(object):
    ...
    def login(self, email=None, password=None, authSubToken=None):
        ...
        if (authSubToken is not None):
            ...
            response = requests.post(self.URL_LOGIN, data=params, headers=headers, verify=False, proxies=proxyDict)
    ...
    def executeRequestApi2(self, path, datapost=None, post_content_type="application/x-www-form-urlencoded; charset=UTF-8"):
        if (datapost is None and path in self.preFetch):
            ...
            # print url
            if datapost is not None:
                response = requests.post(url, data=datapost, headers=headers, verify=False, proxies=proxyDict)
    ...
    def search(self, query, nb_results=None, offset=None):
        ...
        # path = "search?q=%s" % requests.utils.quote(query) # TODO handle categories
    ...
    def download(self, packageName, versionCode, offerType=1):
        ...
        response = requests.get(url, headers=headers, cookies=cookies, verify=False, proxies=proxyDict)
